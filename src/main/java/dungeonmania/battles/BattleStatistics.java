...
public class BattleStatistics {
    ...
    public static List<BattleRound> battle(BattleStatistics self, BattleStatistics target) {
        ...
        if (self.invincible ^ target.invincible) {
            rounds.add(invincibleSituation(self, target));
            return rounds;
        }

        while (self.getHealth() > 0 && target.getHealth() > 0) {
            rounds.add(noInvincibleSituation(self, target));
        }
        return rounds;
    }
    ...
    private static BattleRound invincibleSituation(BattleStatistics self, BattleStatistics target) {
        ...
        return new BattleRound(-damageOnSelf, -damageOnTarget);
    }

    private static BattleRound noInvincibleSituation(BattleStatistics self, BattleStatistics target) {
